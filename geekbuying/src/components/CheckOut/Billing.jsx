...
export const Billing = () => {
  ...
  useEffect(()=>{
    ...
    const getPhoneCode= async ()=>{
      let res= await fetch("https://countriesnow.space/api/v0.1/countries/codes");
      let data = await res.json();
      phones=data.data;
      phones.forEach(elem=>{
  ...
  return  (
    ...
    <tr><td className={styles.left_content}><span className={styles.astrick}>*</span>Zip/Postal Code:</td><td><input placeholder='Your ZIP/postal code' className={styles.Input} type={"text"} value={postal} onChange={(e)=> setPostal(e.target.value)} /></td></tr>
    
    <tr><td className={styles.left_content}><span className={styles.astrick}>*</span>Address line1:</td><td><input className={styles.Input} type={"text"} placeholder="Street address, company name, P.O. box, c/o etc" maxLength="100" value={line1}  onChange={(e)=> setLine1(e.target.value)} /></td></tr>
    
    <tr><td className={styles.left_content}>Address line2:</td><td><input className={styles.Input} type={"text"}  placeholder="Apartment, suite, unit, building, floor etc(Optional)" value={line2} onChange={(e)=> setLine2(e.target.value)} /></td></tr>
    
    <tr><td className={styles.left_content}><span className={styles.astrick}>*</span>Phone number:</td><td><div className={styles.phone_div}><p className={styles.code}>{dial}</p><input className={styles.phone} placeholder='Your phone number' type={"tel"} value={tel} onChange={(e)=> setTel(e.target.value)} /></div></td><td className={styles.note}>Note:Reuired by courier/Postman to ensure delivery and prevent delays. Please enter your correct number.</td></tr>

   <tr><td></td><td><input type={"checkbox"} className={styles.checkbox} />Use as a default</td></tr>
   </tbody>
   </table>
   </form>
    ...
    <div className={styles.first_row}>
    <span className={styles.left}><input type={"radio"} checked={true} onChange={handleChange} />Expedited Shipping</span>
    ...
    <input type={"checkbox"} id="insurance" onClick={handleCheckbox} /> <span>Add Shipping Insurance to your order</span>
    <FontAwesomeIcon className={styles.exclamation} icon="circle-exclamation"/>
    <b>₹​ 816</b>
    <p className={styles.second_note}><span className={styles.astrick}>*</span>Note: The time mentioned above are the shipping days by couriers, not including the order processing days by our warehouse.</p>
    <div className={styles.third_row}>
    <input type={"checkbox"} id="return" onClick={handleCheckbox} /><span>Add EU Local Return Service to your order</span>
    <FontAwesomeIcon className={styles.exclamation} icon="circle-exclamation"/>
    <b>₹​ 520</b>
    </div>
    </div>
    ...
    <div><input  className={styles.coupon_input} id="coupon" name='query' value={query} onChange={(e)=> setQuery(e.target.value)} placeholder='please choose or enter your coupon code'/></div>
    <div><input className={styles.apply} type={"button"} value={"Apply"} onClick={handleCoupon} /></div>
    </div>
    </div>
